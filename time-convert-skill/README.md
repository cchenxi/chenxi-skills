...
```bash
cp time-convert /usr/local/bin/time-convert
...
```
python3 time-convert <input> [-z 时区] [-f 格式] [-e]
```
...
$ python3 time-convert '2026-04-26 09:00:00' --tz America/New_York
$ python3 time-convert '2026-04-26 21:00:00' --tz +9
```
...
# 只需要时间戳
$ python3 time-convert '2026-04-26 21:00:00' -f timestamp
1777208400
```

可选值：`timestamp`、`hex`、`escaped`、`bytes`、`time`、`all`（默认）。
...
```bash
$ python3 time-convert 'prefix_\x69\xEE\x0C\x50_data_\x67\x89\xAB\xCD' -e
...
$ python3 ~/.claude/skills/rowkey-convert/rowkey-convert \
...
18 个用例覆盖正向转换、反向转换（4 种输入）、时区（3 种写法）、格式筛选、提取模式、紧凑格式。
